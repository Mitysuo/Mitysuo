# Vinícius Mitsuo Furukava

<div style="text-align: center;">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=26&duration=3000&pause=1000&color=0A66C2&center=true&vCenter=true&width=700&lines=Dados%2C+IA+e+Mercado+Financeiro;Matem%C3%A1tica+Aplicada+%7C+FGV;Analytics+e+Intelig%C3%AAncia+Artificial+%7C+FIA;Transformando+ideias+em+projetos" alt="Tecnologias e interesses de Vinícius" />
</div>

> Dados, tecnologia e mercado financeiro. Aprendendo, testando e criando projetos com foco em análise, automação e inteligência artificial.

Sou formado em Matemática Aplicada pela FGV, pós-graduado em Analytics e Inteligência Artificial pela FIA e atualmente curso pós-graduação em Operador de Mercado Financeiro, também pela FIA.

Tenho explorado cada vez mais o universo da programação em projetos pessoais e profissionais, principalmente em áreas ligadas a dados, automação, análise financeira e inteligência artificial.


## 👨‍💻 Sobre mim

- Interesse em dados, mercado financeiro, inteligência artificial e tecnologia.
- Aspirante a maratonista.
- Interessado em cultura asiática.
- Buscando transformar ideias em projetos práticos.

## 🎯 Foco atual

- Python aplicado a análise de dados e automação.
- Machine learning e inteligência artificial aplicada.
- Análise financeira, mercado de capitais e visualização de dados.
- Organização de estudos e projetos.

## 🛠️ Tecnologias e ferramentas

<div style="text-align: center;">

[![My Skills](https://skillicons.dev/icons?i=py,vscode,sklearn,html,css,docker,git,github,latex,matlab,notion,obsidian&perline=6)](https://skillicons.dev)

</div>

## 🧭 Áreas de interesse

- Análise de dados
- Inteligência artificial
- Machine learning
- Automação de processos
- Mercado financeiro
- Visualização de dados
- Projetos pessoais com Python

## 📫 Contato

Se quiser trocar uma ideia sobre tecnologia, dados, mercado financeiro ou projetos:

<div style="text-align: center;">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Vinicius%20Furukava-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/vinicius-furukava/)
[![GitHub](https://img.shields.io/badge/GitHub-Mitysuo-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Mitysuo)

</div>